...
# Section 4: Phylogenetic Analyses
```{r, include=FALSE}
...
seqmat.long <- seqmat %>% 
  ...
  dplyr::mutate(POS = factor(POS, levels = c(1:5898), ordered = T)) 
...
jpege("~/Documents/GitHub/VivID_Seq/PopGenome_Analysis/figures/gtplot.jpg", height = 8, width = 11, units = "in", res = 500)
plot(gtplot)
graphics.off()
...
iters <- 1e1
bs <- phangorn::bootstrap.pml(fitGTR, bs=iters, 
                              optNni=TRUE, optBf=TRUE, optQ=TRUE, optGamma=TRUE)
