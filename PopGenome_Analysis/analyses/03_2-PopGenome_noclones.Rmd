...
# Genetic Statistics, Only Unique Haplotypes
```{r, include=FALSE}
...
```{r}
mtdna <- PopGenome::readData(path = "~/Documents/GitHub/VivID_Seq/PopGenome_Analysis/data/noclonefasta/",
                             format = "fasta",
                             include.unknown = T,
...
mtdna.struct <- PopGenome::set.outgroup(mtdna.struct,
                                        c("Pcynomolgi"))
...
#                                            pi = T)
# Can't do within diversity stats if DRC only has one population                                           
...
### Derived Sites
```{r}
mtdnaseq <- Biostrings::readDNAStringSet(filepath = "~/Documents/GitHub/VivID_Seq/PopGenome_Analysis/data/noclonefasta/unique_mtdna.fa")

seqmat <- as.matrix(mtdnaseq)
anc <- seqmat[rownames(seqmat) %in% "Pcynomolgi", ]
smpls.seq.list <- seqmat[!rownames(seqmat) %in% "Pcynomolgi", ]
...
**Can't be completed as DRC only has one unique haplotype**. 
